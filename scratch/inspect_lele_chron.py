...
# Filter Lê Lê kể chữ
lele_vids = [v for v in media_videos if v["category"] == "Lê Lê kể chữ"]
# Sort by order descending
lele_vids.sort(key=lambda x: x["order"], reverse=True)

print(f"=== Category: Lê Lê kể chữ (Total: {len(lele_vids)}) ===")
for v in lele_vids:
